...
class AntColony(object):
    ...
    def __init__(self, distances, n_ants, n_best, n_iterations, decay, alpha=1, beta=1):
        ...
        self.n_ants = n_ants  # Кількість мурах, що запускаються за ітерацію
        self.n_best = n_best  # Кількість найкращих мурах, які відкладають феромон
        self.n_iterations = n_iterations  # Кількість ітерацій
        self.decay = decay  # Швидкість розпаду феромону
        self.alpha = alpha  # Показник для феромону, вищий alpha надає феромону більшеї ваги
        self.beta = beta  # Показник для відстані, вищий beta надає відстані більшеї ваги
...
result = ant_colony.run(22)
print(f"Відстань: {result[1]}")

path = "Шлях: "
for i in result[0]:
    path += f"{cities[i[0]]} -> "
path += f"{cities[result[0][-1][1]]}"
print(path)
...
plt.plot([i + 1 for i in range(len(result[0]))], [i[0] for i in result[0]], ms=10, marker='.', mfc='red', mew=2,
         color='red')
plt.grid()
plt.show()
